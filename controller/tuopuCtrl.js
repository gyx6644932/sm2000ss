;(function(app){
    app.controller('tuopuCtrl',['$scope','$rootScope','$state', "$timeout", 'publicService',"ngDialog", "areaData",function($scope, $rootScope, $state, $timeout, publicService,ngDialog,areaData){
        //滚动鼠标地图放大缩小
        ...
        $scope.dMouseDown = function(){
            ...
            this.xy = {x : event.pageX || window.event.pageX, y : event.pageY || window.event.pageX};
        }
        //修改添加拓扑图设备
        ...
        $scope.treeFun = function(sel) {
            ...
            if(sel.flag && sel.flag === "device"){
                if(sel.deviceStatus !== 0){
                    ...
                    /*右上角接口*/
                    publicService.doRequest("GET", "/nms/spring/deviceInfo/" + devId + "/getGeneralStatus", {}).success(function(r){
                        if(r.data && r.data.ccStateTable && r.data.generalStatusNoTrap ){
                            var m1 = r.data.generalStatusNoTrap[0],
                                timeInter = new Date(m1.mcTime.trim().substring(0,19)).getTime();
                            function timef(){
                                timeInter += 1000; 
                                $scope.mcTime = timeInter;
                                $timeout(timef,1000);
                            }
                            timef();
                            $scope.mcTime = m1.mcTime.trim().substring(0,19);
                            $scope.systemName = m1.systemName;
                            $scope.refMode = m1.refMode;
                            $scope.refInput = m1.refInput;
                            $scope.clockStatus = m1.clockStatus;
                            $scope.ccstate1 = devicecc12(r.data.ccStateTable["0"].ccState);
                            $scope.ccstate2 = devicecc12(r.data.ccStateTable["1"].ccState);
                    ...
                    function loadframes(x){
                        publicService.doRequest("GET", "/nms/spring/deviceInfo/" + devId + "/getIoStatus", {shelfIndex : x}).success(function(r){
                            $scope.frameInfoList = r.data;
                        })
                    }

                    publicService.doRequest("GET", "/nms/spring/devices/parents/" + devId, {}).success(function(r){
                        var arr = [];
                        if(r.status === "success"){
                            var rr = r.data;
                            for (var i = 0; i < rr.length; i++) {
                                if(rr[i].deviceStatus === 1){
                                    arr.push({v:macFramefun(rr[i].name),t:rr[i].name === "MainShelf" ? "主机框": rr[i].name})
                                }
                            }
                    ...
                    publicService.ngAlert('该设备处于离线离线状态！');
                    return;
                }
            ...
            if(sel.flag === null){
                if(sel.deviceStatus !== 0){
                    publicService.loading('start');
                    var devId = sel.parentId;
                    publicService.doRequest("GET", "/nms/spring/deviceInfo/" + devId + "/getCardLed", {shelfIndex : macFramefun(sel.name)}).success(function(r){
                        publicService.loading('end');
                        if(r.data.length > 0){
